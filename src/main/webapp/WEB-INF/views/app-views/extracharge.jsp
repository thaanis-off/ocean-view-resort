...
<head>
    ...
    <title>${charge != null ? 'Edit' : 'Add'} Incidental Charge — Ocean View Resort</title>
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside class="w-64 bg-gray-900 border-r border-gray-800 flex flex-col">
            ...
            <nav class="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
                ...
                <a href="${pageContext.request.contextPath}/reservation?action=list" class="flex items-center px-4 py-3 text-sm font-medium text-white bg-gray-800 rounded-lg">
            ...
            <header class="bg-gray-900 border-b border-gray-800 px-8 py-4">
                <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-4">
                        <a href="${pageContext.request.contextPath}/reservation?action=view&id=${reservation.id}" class="text-gray-400 hover:text-white transition-colors">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
                        </a>
                        <div>
                            <h1 class="text-2xl font-bold text-white flex items-center">
                                ${charge != null ? 'Edit Charge' : 'Record Incidental Charge'}
                                <span class="ml-3 px-2.5 py-0.5 rounded-full text-xs font-medium bg-primary-500/20 text-primary-400 border border-primary-500/30">
                                    ${charge != null ? 'Update Record' : 'New Post to Tab'}
                                </span>
                            </h1>
                            <p class="text-sm text-gray-400 mt-1">Add food, spa, or other extra charges to the guest's folio.</p>
                        </div>
                    </div>
                    <div class="flex items-center space-x-4">
            ...
            <main class="flex-1 overflow-y-auto bg-gray-950 p-8">
                ...
                <div class="max-w-4xl mx-auto">
                    ...
                    <c:if test="${not empty reservation}">
                        <div class="bg-blue-500/10 border border-blue-500/20 rounded-xl p-5 mb-8 flex items-start shadow-sm">
                            <svg class="w-6 h-6 text-blue-400 mr-4 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                            <div>
                                <h4 class="text-base font-semibold text-blue-300">Adding charge to Reservation: ${reservation.reservationNumber}</h4>
                                <p class="text-sm text-blue-400/80 mt-1">Guest: <strong class="text-blue-300">${reservation.guestName}</strong> (Staying in Room ${reservation.roomNumber})</p>
                            </div>
                        </div>
                    </c:if>
                    ...
                    <div class="bg-gray-900 border border-gray-800 rounded-xl shadow-lg p-8">
                        <form action="${pageContext.request.contextPath}/extracharge" method="post" class="space-y-8">
                            ...
                            <div>
                                <h3 class="text-lg font-semibold text-white border-b border-gray-800 pb-3 mb-6">Charge Details</h3>
                                
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                    <div>
                                        <label class="block text-sm font-medium text-gray-400 mb-2">Category <span class="text-red-500">*</span></label>
                                        <select name="category" required class="w-full bg-gray-950 border border-gray-700 rounded-lg px-4 py-2.5 text-white focus:border-primary-500 focus:ring-1 focus:ring-primary-500 focus:outline-none transition-colors appearance-none">
                                            <option value="" class="bg-gray-900">-- Select Category --</option>
                                            <option value="Food" ${charge != null && charge.category == 'Food' ? 'selected' : ''} class="bg-gray-900">Food & Beverage</option>
                                            <option value="Spa" ${charge != null && charge.category == 'Spa' ? 'selected' : ''} class="bg-gray-900">Spa & Wellness</option>
                                            <option value="Laundry" ${charge != null && charge.category == 'Laundry' ? 'selected' : ''} class="bg-gray-900">Laundry</option>
                                            <option value="Excursion" ${charge != null && charge.category == 'Excursion' ? 'selected' : ''} class="bg-gray-900">Excursion / Tours</option>
                                            <option value="Other" ${charge != null && charge.category == 'Other' ? 'selected' : ''} class="bg-gray-900">Other</option>
                                        </select>
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-gray-400 mb-2">Amount (LKR) <span class="text-red-500">*</span></label>
                                        <input type="number" name="amount" step="0.01" min="0" value="${charge != null ? charge.amount : param.amount}" placeholder="0.00" required
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                    <div>
                                        <label class="block text-sm font-medium text-gray-400 mb-2">Charge Date <span class="text-red-500">*</span></label>
                                        <input type="date" name="chargeDate" value="${charge != null ? charge.chargeDate : param.chargeDate}" required
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-gray-400 mb-2">Description <span class="text-red-500">*</span></label>
                                    <textarea name="description" placeholder="E.g., Dinner at main restaurant, 2x cocktails..." rows="3" required
                                        class="w-full bg-gray-950 border border-gray-700 rounded-lg px-4 py-3 text-white focus:border-primary-500 focus:ring-1 focus:ring-primary-500 focus:outline-none transition-colors resize-y">${charge != null ? charge.description : param.description}</textarea>
                                </div>
                            </div>

                            <div class="pt-6 mt-6 border-t border-gray-800 flex items-center justify-end space-x-4">
                                <a href="${pageContext.request.contextPath}/reservation?action=view&id=${reservation.id}" class="px-6 py-2.5 bg-gray-800 hover:bg-gray-700 text-gray-300 font-medium rounded-lg border border-gray-700 transition-colors">
                                    Cancel
                                </a>
                                <button type="submit" class="px-6 py-2.5 bg-gradient-to-br from-primary-600 to-gray-900 hover:from-primary-500 hover:to-gray-800 border border-primary-500/30 text-white font-medium rounded-lg shadow-md transition-all duration-300">
                                    ${charge != null ? 'Update Charge' : 'Add to Tab'}
                                </button>
                            </div>
